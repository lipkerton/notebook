...
```
Проблема одна - я использую не `init.vim` , а `init.lua` и при этом я не шарю как написать на языке lua то, что было написано на языке vim. То есть вот эти команды сверху нужно переписать на lua. Как это сделать? В душе не ебу. Но спустя два часа поиска появился [спаситель](https://stackoverflow.com/a/70207074).
```Lua
