...
```
Следующий важный параметр - `agent`. Он определяет локацию, где будет выполняться наш пайплайн. Вариантов может быть несколько, но я использую один из самых простых для начала: `any`, `label`, `node`.
- `any` - любая доступная среда выполнения.
- `label` - простое указание метки среды запуска. У Jenkins есть каталог меток, чтобы можно было просто указать имя метки и остальное выполнилось через этот каталог. Список меток есть в `Настроить Jenkins -> Nodes`. Например у меня доступна одна метка - `мастер`:
```Groovy
pipeline {
	agent {
		label "мастер"
