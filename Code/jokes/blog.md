...
На данном этапе материала не много - я описал его [[blog#тесты пользователя|здесь]].
...
```
# Jenkins
Подробнее про Jenkins я описал тут: [[Jenkins]].
Здесь я расскажу о том, что нужно сделать, когда я хочу составить пайплайн для текущего проекта.
Для пайплайна я составил такой `Jenkinsfile`:
```Groovy
pipeline {
    agent {
        label "debian"
...
```
Но с ним появляются свои проблемы:
- Запуск команд `Docker` требует `sudo`.
- `Docker` запрещает скачивать образы с DockerHub без аутентификации.
Чтобы решить первую проблему следует дать возможность пользователю `jenkins` запускать команды `docker`. Но если добавить его в `sudoers`, то автоматический пайплайн повиснет на требовании пароля + это просто небезопасно.
Следует понимать, что любые команды в командной строке, которые забивают I/O, например, запуск сервера `uvicorn`, мешают пайплайну двигаться вперед и, следовательно, он просто виснет.
Чтобы запускать команды Docker не от `sudo`, нужно добавить пользователя `jenkins` в группу `docker`:
```Bash
sudo usermod -aG docker jenkins
```
А чтобы решить проблему со скачкой образов нужно аутентифицироваться в Docker:
```Bash
