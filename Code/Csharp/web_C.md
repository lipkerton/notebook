...
- **Инкапсуляция (Encapsulation)** - инкапсуляция означает набор правил для создания экземпляров данного класса, которые ограничивают определенным образом процесс инициализации его объектов. Например, выше я написал два конструктора и один сразу сделал приватным и это пример работы инкапсуляции - я запретил создавать пустые сущности на основании класса, чтобы не загрузить в БД пустые объекты. Такие запреты в разных форматах всегда гарантируют, что объекты класса будут находиться в валидном состоянии.
- **Инварианты (Invariants)** - это набор правил, который поддерживает целостность данных, которые используются при создании объекта данного класса. Например, когда я делаю методы, которые проверяют, что переменные в классе содержат корректные значения и т.д.
- **Богатая модель данных (Rich domain model)** - в контексте создания моделей существует два подхода:
	- *Anemic domain model* - когда класс, где описаны поля будущей таблицы в БД, содержит только описания полей и не содержит никакой бизнес-логики.
	- *Rich domain model* - когда класс, где описаны поля будущей таблицы в БД, содержит бизнес-логику внутри себя, т.е. он валидирует данные по определенным правилам и может *умно* подходить к созданию объектов на своей основе.
- **Иммутабельность свойств (Property immutability)** - в примере кода выше каждое свойство класса обладает приватным сеттером. Это означает, что свойство не может быть "случайно" изменено невалидным значением извне класса, но может быть установлено с помощью методов самого этого класса. Это гарантирует безопасность значений, которые лежат в свойствах.
# Currency
В предыдущей главе я написал класс `SaleCommission`, где использовался класс `Currency`, который у меня еще не описан. Сейчас я хочу написать класс `Currency`:
```C#
...
```
Последняя строка переоформляет метод `ToString()`, который по умолчанию есть у каждого объекта в C#. Причем такая запись является сокращенной записью:
```C#
// можно написать так:
public override string? ToString() => Code;
...
<<<<<<<< Updated upstream:Code/Csharp/web_C.md
---
Допишу еще одну строку:
```C#
public override int GetHashCode() => Code.GetHashCode();
```
Она переоформляет поведение еще одного метода - `GetHashCode()`. Грубо говоря этот метод возвращает числовое представление об объекте. В данном случае я хочу, чтобы в результате работы метода вернулось числовое представление объекта `Code`.
========
>>>>>>>> Stashed changes:Code/C/web_C/DDD.md
